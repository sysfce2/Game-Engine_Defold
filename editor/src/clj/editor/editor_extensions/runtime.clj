...
(defn current-execution-context
  "Returns the current execution context, a map with following keys:
    ...
    :rt                    the EditorExtensionsRuntime used for execution
...
(defn- resolve-file [project ^String file-path]
  (let [{:keys [evaluation-context]} (current-execution-context)
        project-path (-> project
                         (project/workspace evaluation-context)
                         (workspace/project-path evaluation-context)
                         .toPath
                         .normalize)
        target-path (.normalize (.resolve project-path file-path))]
...
(defn make
  ...
  (let [vm (vm/make)
        ...
        globals (doto (vm/env vm)
                  ...
                  (.load (CoroutineLib.))
                  (.load (JseMathLib.))
                  (.load (DefoldIoLib. #(resolve-file project %)))
                  (.load (JseOsLib.))
                  (LoadState/install)
                  (LuaC/install)
