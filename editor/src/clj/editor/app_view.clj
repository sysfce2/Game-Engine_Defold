  ...
  (let [hook-opts {:url url}]
    (future
      (error-reporting/catch-all!
        (try
          @(extensions/execute-hook! project :on_target_launched hook-opts)
          (catch Exception _))
        (process/on-exit! process #(future/catch
                                     (extensions/execute-hook! project :on_target_terminated hook-opts)
          ...
          (let [platform (engine/current-platform)
            ...
            (run-on-background-thread!
              (fn run-post-build-hook-on-background-thread! []
                (try
                  @(extensions/execute-hook! project :on_build_finished {:success project-build-successful :platform platform})
                  (catch Exception _)))
          ...
          (let [platform (engine/current-platform)]
            (run-on-background-thread!
              (fn run-pre-build-hook-on-background-thread! []
                (let [extension-error
                      (try
                        @(extensions/execute-hook! project :on_build_started {:platform platform})
                        (catch Exception e (extensions/hook-exception->error e project :on_build_started)))]
                  ...
                  (when (some? extension-error)
                    (render-progress! (progress/make-indeterminate "Executing post-build hooks..."))
                    (try
                      @(extensions/execute-hook! project :on_build_finished {:success false :platform platform})
                      (catch Exception _)))
                  extension-error))
...
#_(defn make-extensions-ui [workspace changes-view prefs]
    (reify extensions/UI
      (reload-resources! [_]
